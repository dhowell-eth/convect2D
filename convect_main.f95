...
program convect_main
  ...
  real :: dt_for_write,c,beta
  real, allocatable :: frame_times(:),rhs(:,:),new_field(:,:)

  ! Read in input parameters from a file
  namelist /inputs/ nx,ny,total_time,diffusivity,a_dif,a_adv,ra,pr,error_threshhold,num_frames,beta
  ...
  allocate(this_advection_grid%T(nx,ny),this_advection_grid%phi(nx,ny))
  allocate(this_advection_grid%v_x(nx,ny),this_advection_grid%v_y(nx,ny))
  ...
  allocate(rhs(nx,ny),new_field(nx,ny))
  ...
  dt_dif = a_dif*(this_advection_grid%h**2)/max(1.0,pr)
  ...
  do
    ...
    do while (rms_residue/rms_f>error_threshhold)
      rms_residue = Vcycle_2DPoisson(s,this_advection_grid%w,this_advection_grid%h,0.0,.false.)
    end do  
    ...
    !-----------------!
    ! Figure out time_step depending on the value of beta
    ...
    if (beta>0.0) then
      ...
      rhs = 0.0
      new_field = 0.0
      new_field = this_advection_grid%T
      rms_f = 0.0

      c=1.0/(beta*dt)
      rhs = (-1.0*c)*( this_advection_grid%T + dt * (  ((1.0-beta) * del2) - vgrad)  ) 
      ...
      do while (rms_residue/rms_f>error_threshhold)
        ...
        !print*,"T loop"
        !print*,rms_f,rms_residue
        rms_residue = Vcycle_2DPoisson(new_field,rhs,this_advection_grid%h,c,.false.)
      end do  

      this_advection_grid%T = new_field
      !print*,MAXVAL(this_advection_grid%T)
      ! --- W ---
      rhs = 0.0
      new_field = 0.0
      new_field = this_advection_grid%w
      rms_f = 0.0

      c=1.0/(pr*beta*dt)
      !print*,this_advection_grid%T(:,1)
      !print*,"c:",c
      rhs = (-1.0*c)*(this_advection_grid%w+dt*(pr*(1.0-beta)*del2_w - vgrad_w - (ra*pr*old_dtdx))  )
      rms_f = rms_2D(rhs)
      rms_residue = rms_f*2.0
      do while (rms_residue/rms_f>error_threshhold)
        !print*,"W loop"
      ...
      this_advection_grid%w = new_field
